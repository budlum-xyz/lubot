...
impl Default for Ayarlar {
    fn default() -> Self {
        Self { sicaklik: 1.0, top_k: 0, top_p: 1.0 }
    }
}
...
pub fn dagilim(logitler: &[f64], ayar: &Ayarlar) -> Result<Vec<(u32, f64)>, OrnekHatasi> {
    ...
    agirlik.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal)
        .then(a.0.cmp(&b.0)));
...
mod testler {
    ...
    fn sicaklik_sifir_argmax_verir() {
        ...
        let ayar = Ayarlar { sicaklik: 0.0, ..Ayarlar::default() };
    ...
    fn sicaklik_orani_olcekler() {
        ...
        for sicaklik in [0.5f64, 1.0, 2.0] {
            ...
            let d = dagilim(&logitler, &Ayarlar { sicaklik, ..Ayarlar::default() }).expect("dagilim");
    ...
    fn top_k_kume_boyutunu_kirpar() {
        ...
        let d1 = dagilim(&logitler, &Ayarlar { top_k: 1, ..Ayarlar::default() }).expect("dagilim");
        assert_eq!(d1.len(), 1);
        assert_eq!(d1[0].0, 0);
        let d3 = dagilim(&logitler, &Ayarlar { top_k: 3, ..Ayarlar::default() }).expect("dagilim");
        assert_eq!(d3.len(), 3);
        assert_eq!(d3.iter().map(|(j, _)| *j).collect::<Vec<_>>(), vec![0, 1, 2]);
    ...
    fn nucleus_en_kucuk_kumeyi_tutar() {
        ...
        let d = dagilim(&logitler, &Ayarlar { top_p: 0.5, ..Ayarlar::default() }).expect("dagilim");
        assert_eq!(d.len(), 1, "0,5 esiginde kume tek jetona inmeliydi");
        let d2 = dagilim(&logitler, &Ayarlar { top_p: 0.99, ..Ayarlar::default() }).expect("dagilim");
        assert_eq!(d2.len(), 2);
    ...
    fn dagilim_toplami_birdir_ve_azalan_siralidir() {
        ...
        let d = dagilim(&logitler, &Ayarlar { sicaklik: 0.8, top_p: 0.9, ..Ayarlar::default() })
            .expect("dagilim");
    ...
    fn ayni_tohum_ayni_diziyi_verir() {
        ...
        let ayar = Ayarlar { sicaklik: 1.0, top_k: 0, top_p: 1.0 };
        let mut a = Rastgele::tohumdan(42);
        let mut b = Rastgele::tohumdan(42);
        let dizi_a: Vec<u32> = (0..64).map(|_| sec(&logitler, &ayar, &mut a).expect("sec")).collect();
        let dizi_b: Vec<u32> = (0..64).map(|_| sec(&logitler, &ayar, &mut b).expect("sec")).collect();
        ...
        let dizi_c: Vec<u32> = (0..64).map(|_| sec(&logitler, &ayar, &mut c).expect("sec")).collect();
    ...
    fn ornekleme_dagilimi_gezer_ama_sinirin_disina_cikmaz() {
        ...
        let ayar = Ayarlar { top_k: 2, ..Ayarlar::default() };
        ...
        assert_eq!(gorulen.iter().copied().collect::<Vec<_>>(), vec![0, 1],
    ...
    fn gecersiz_ayar_ve_bos_girdi_reddedilir() {
        assert!(matches!(
            dagilim(&[1.0], &Ayarlar { sicaklik: -1.0, ..Ayarlar::default() }),
            Err(OrnekHatasi::GecersizAyar(_))
        ));
        assert!(matches!(
            dagilim(&[1.0], &Ayarlar { sicaklik: f64::NAN, ..Ayarlar::default() }),
            Err(OrnekHatasi::GecersizAyar(_))
        ));
        assert!(matches!(
            dagilim(&[1.0], &Ayarlar { top_p: 0.0, ..Ayarlar::default() }),
            Err(OrnekHatasi::GecersizAyar(_))
        ));
        assert!(matches!(
            dagilim(&[1.0], &Ayarlar { top_p: 1.5, ..Ayarlar::default() }),
            Err(OrnekHatasi::GecersizAyar(_))
        ));
        assert!(matches!(dagilim(&[], &Ayarlar::default()), Err(OrnekHatasi::BosLogit)));
        assert!(matches!(
            dagilim(&[f64::NAN, 1.0], &Ayarlar::default()),
            Err(OrnekHatasi::CozulemezLogit)
        ));
        assert!(matches!(ornekle(&[], &mut Rastgele::tohumdan(1)), Err(OrnekHatasi::BosLogit)));
    }
