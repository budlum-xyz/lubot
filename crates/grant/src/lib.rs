...
impl GrantBook {
    ...
    /// Live grants as `(key_id, grantee, expires_at)`, for bookkeeping surfaces.
    #[must_use]
    pub fn live(&self) -> Vec<(&str, &str, Seconds)> {
        let mut out: Vec<(&str, &str, Seconds)> = self
            .live
            .iter()
            .map(|((k, r), exp)| (k.as_str(), r.as_str(), *exp))
            .collect();
        out.sort_unstable();
        out
    }

    /// Revoked pairs as `(key_id, grantee)`, for the same bookkeeping surfaces.
    #[must_use]
    pub fn revoked_pairs(&self) -> Vec<(&str, &str)> {
...
mod tests {
    ...
    fn the_book_exposes_live_and_revoked_for_bookkeeping() {
        ...
        assert_eq!(live, vec![("dm-2", "reader", 200)]);
        assert_eq!(revoked, vec![("dm-1", "reader")]);
    }
}
