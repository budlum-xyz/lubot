...
558 tests, `clippy -D warnings` clean, `unwrap`/`expect` denied outside tests. 86 gates, each with its own self-test; the ratchet holds on seven keys: tests, gates, pedantic warnings, corpus records, corpus tokens, bootstrap rounds, exam questions. The corpus-derived numbers are deliberately *not* restated here. This file is part of the corpus, so a figure written here feeds back into the measurement that produced it - measured, not assumed: with 101443 written here the corpus measures 101444, and with 101444 written it measures 101443, a two-cycle with no fixed point. `training/ratchet.json` carries them and is not in the corpus.
