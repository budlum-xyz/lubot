...
| crate | lines | tests | what it holds |
|---|---|---|---|
| `cli` | 11298 | 147 | The runnable binary, and the only crate that reaches everything else. `kodlayici envanter` prints what is inside a package and which tensors nothing read; `kodlayici kosu` runs the encoder and, with `--isaret`, the head; `kodlayici dogrula` streams the parts through SHA-256 and compares the digest with one given, so a run can say which artifact it ran. |
