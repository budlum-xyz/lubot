...
| `esik` | 537 | 13 | A quorum over a set that includes the requester is not a quorum. `n >= 3f + 1`. A threshold that would drop below the Byzantine floor is refused, so `5 of 7` cannot become `5 of 40`. |
...
|---|---|---|---|---|
| `denetim` | 585 | 14 | `muhur` | An append-only audit trail. There is **no update method and no delete method**; corrections are appended entries. The chain detects edits and names the first index, but **cannot detect truncation** - `head()` has to be anchored elsewhere, and `verify_against_anchor` is what catches it. |
...
| crate | lines | tests | what it holds |
|---|---|---|---|
| `cli` | 7045 | 98 | The runnable binary, and the only crate that reaches everything else. |
